...
impl OptionDescriptor {
    pub fn int(name: &str, label: &str, default: i64, min: i64, max: i64, rebuilds_state: bool) -> Self {
        Self {
            name: name.to_string(),
    ...
    pub fn float(name: &str, label: &str, default: f64, min: f64, max: f64, rebuilds_state: bool) -> Self {
        Self {
            name: name.to_string(),
            label: label.to_string(),
    ...
    pub fn choice(name: &str, label: &str, default: &str, choices: Vec<&str>, rebuilds_state: bool) -> Self {
        Self {
            name: name.to_string(),
            label: label.to_string(),
    ...
    pub fn default(&self) -> &OptionValue { &self.default }
    pub fn kind(&self) -> &OptionKind { &self.kind }
    pub fn rebuilds_state(&self) -> bool { self.rebuilds_state }

    fn validate(&self, preset: &str, value: OptionValue) -> Result<OptionValue> {
        match (&self.kind, value) {
            (OptionKind::Int { min, max }, OptionValue::Int(value)) if value >= *min && value <= *max => Ok(OptionValue::Int(value)),
            (OptionKind::Int { min, max }, OptionValue::Int(value)) => Err(AsciiAnimError::OptionOutOfRange {
                option: self.name.clone(),
                min: min.to_string(),
                max: max.to_string(),
                actual: value.to_string(),
            }),
            (OptionKind::Float { min, max }, OptionValue::Float(value)) if value >= *min && value <= *max => Ok(OptionValue::Float(value)),
            (OptionKind::Float { min, max }, OptionValue::Float(value)) => Err(AsciiAnimError::OptionOutOfRange {
                option: self.name.clone(),
                min: trim_float(*min),
                max: trim_float(*max),
            ...
            (OptionKind::Choice { choices }, OptionValue::Choice(value)) if choices.contains(&value) => Ok(OptionValue::Choice(value)),
            (OptionKind::Choice { choices }, OptionValue::Choice(value)) => Err(AsciiAnimError::InvalidChoice {
                option: self.name.clone(),
...
impl PresetDescriptor {
    pub fn new(name: &str, label: &str, description: &str, options: Vec<OptionDescriptor>) -> Self {
        Self {
            name: name.to_string(),
            label: label.to_string(),
    ...
    pub fn options(&self) -> &[OptionDescriptor] { &self.options }
    ...
    pub fn validate_options(&self, raw: &BTreeMap<String, OptionValue>) -> Result<BTreeMap<String, OptionValue>> {
...
impl PresetRegistry {
    ...
    pub fn get(&self, name: &str) -> Result<&PresetDescriptor> {
        self.presets.get(name).ok_or_else(|| AsciiAnimError::UnknownPreset {
            name: name.to_string(),
        })
    }
